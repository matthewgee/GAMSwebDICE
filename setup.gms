...
** Preferences
 B_ELASMU   EFlasticity of marginal utility of consumption     /  2   /
...
PARAMETERS
  ...
  SIGMA(T)      Exogenous CO2-equivalent-emissions output ratio
...
VARIABLES
 ...
 GNU(T)          Endogenous part of CO2-equivalent-emissions output ratio
 X(T)            R&D Investment
 SIGMA_N(T)      Combined  CO2-equivalent-emissions output ratio
...
POSITIVE VARIABLES MIU, TATM, TOCE, E, MAT, MATAV, MU, ML, Y, YGROSS, C, K, X,I, GNU, CCA ;
...
* KC(T)            Terminal condition for capital
 ...
 ECOST(T)         Abatement cost function
* ETERM         Terminal emmissions condition
...
*KC(TLAST)..        K(TLAST) =E= 6541;
EE(T)..            E(T)=E=10*SIGMA_N(T)*(1-MIU(T))*YGROSS(T) + ETREE(T);
Ecost(T)..         cost1(T) =e= (PBACK*SIGMA(T)/EXPCOST2)* ( (BACKRAT-1+ EXP (-gback* (ORD(T)-1) ) )/BACKRAT);
*ETERM..            E(40)=L= E(39);
FORCE(T)..         FORC(T) =E=  FCO22X*((log((Mat(T)+.000001)/596.4)/log(2)))+FORCOTH(T);
